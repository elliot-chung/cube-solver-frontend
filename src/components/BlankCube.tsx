...
function BlankCube({ setCubeData, activeColor }: BlankCubeProps) {
  ...
  const setColor = useCallback(
    (type: string, id: number, normalAxis: string) => {
      if (type === "edge") {
        const newEdgeColors = [...edgeColors]
        newEdgeColors[id][normalAxis === "x" ? 0 : normalAxis === "y" ? 1 : 2] =
          activeColor
        setEdgeColors(newEdgeColors)
      } else if (type === "corner") {
        const newCornerColors = [...cornerColors]
        newCornerColors[id][normalAxis === "x" ? 0 : normalAxis === "y" ? 1 : 2] =
          activeColor
      ...
      Array(12)
        ...
        .map((_, i) => (
          <Cubie
            ...
            colors={edgeColors[i]}
            key={"e" + i}
            setColor={(axis: string) => setColor("edge", i, axis)}
          />
        )),
    [edgeColors, activeColor]
  )
      ...
      Array(8)
        ...
        .map((_, i) => (
          <Cubie
            ...
            key={"c" + i}
            setColor={(axis: string) => setColor("corner", i, axis)}
          />
        )),
    [cornerColors, activeColor]
  )
  const centerCubies = Array(6)
    .fill(0)
    .map((_, i) => (
      <Cubie type="center" positionId={i} colors={centerColors[i]} key={"m" + i} />
    ))
